...
func shouldLabelMySQLSELinux(ctx *runner.StepContext, mode string) bool {
	...
	case mysqlSELinuxLabel:
		state := strings.ToLower(commonos.GetSELinuxEnforce(ctx))
...
// stepSelinuxContext labels mysql tree when SELinux is active.
func stepSelinuxContext() *runner.Step {
	return &runner.Step{
		...
		PreCheck: func(ctx *runner.StepContext) error {
			...
			if !shouldLabelMySQLSELinux(ctx, mode) {
				return fmt.Errorf("mysql selinux labeling skipped (mode=%s, getenforce=%s)", mode, commonos.GetSELinuxEnforce(ctx))
			}
			return nil
		},

...
// stepInstallSoftware 安装软件到备库节点步骤
func stepInstallSoftware() *runner.Step {
	return &runner.Step{
		...
		PreCheck: func(ctx *runner.StepContext) error {
			// 同机扩容已绑定 host-id：跳过 host add（即使 stage 残留 hosts_add.toml）
		...
		Action: func(ctx *runner.StepContext) error {
			...
			if hid := strings.TrimSpace(ctx.GetParamString("standby_host_id", "")); hid != "" {
				ctx.Logger.Info("Same-host expansion (host-id=%s): skip yasboot host add (reuse existing host software/agent)", hid)
				standbyLogPhase(ctx, "install-done", "skip host add (same-host)")
				return nil
			}
			stageDir := ctx.GetParamString("db_stage_dir", "/home/yashan/install")
			hostsAddFile := fmt.Sprintf("%s/hosts_add.toml", stageDir)

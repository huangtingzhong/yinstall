// e004_check_replication_addr.go - 检查主库 replication_addr 参数
...
func StepE004CheckReplicationAddr() *runner.Step {
	return &runner.Step{
		ID:          "E-004",
		...
		Action: func(ctx *runner.StepContext) error {
			...
			if !isConfigured {
				...
				ctx.Logger.Error("Configure on the primary:")
				ctx.Logger.Error("  1. Connect as SYS: yasql sys/<password>@%s", clusterName)
				ctx.Logger.Error("  2. ALTER SYSTEM SET REPLICATION_ADDR = '<IP>:<PORT>';")
				ctx.Logger.Error("     Example: ALTER SYSTEM SET REPLICATION_ADDR = '192.168.1.100:1688';")
				ctx.Logger.Error("  3. Verify: SELECT name, value FROM v$parameter WHERE name = 'REPLICATION_ADDR';")
				ctx.Logger.Error("Use the primary IP and port that standby hosts can reach.")
				ctx.Logger.Error("================================================================")

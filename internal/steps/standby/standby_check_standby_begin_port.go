...
// stepCheckStandbyBeginPort 在每台备库上检查 db_begin_port（与 --db-port 一致）是否已被占用
func stepCheckStandbyBeginPort() *runner.Step {
	...
	return &runner.Step{
		...
		PreCheck: func(ctx *runner.StepContext) error {
			...
			for _, th := range ctx.HostsToRun() {
				if err := checkOnHost(ctx, th, beginPort); err != nil {
					return err
				}
			}
			return nil

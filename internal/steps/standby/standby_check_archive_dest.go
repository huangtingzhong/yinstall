...
// stepCheckArchiveDest 检查归档路径是否已包含目标端步骤
func stepCheckArchiveDest() *runner.Step {
	return &runner.Step{
		...
		PreCheck: func(ctx *runner.StepContext) error {
			return checkArchiveDest(ctx)
		},

		Action: func(ctx *runner.StepContext) error {
			standbyLogPhase(ctx, "plan", "Check Archive Destination")
			return checkArchiveDest(ctx)
		},
...
// checkArchiveDest 只读：归档目的地 vs 目标 IP，并结合 cluster status 判定是否已配置。
func checkArchiveDest(ctx *runner.StepContext) error {
	...
	for _, hit := range archiveHits {
		...
		if beginPort > 0 {
			openPort := ListenPortFromAddress(row.ListenAddress)
			if openPort > 0 && openPort != beginPort {
				ctx.Logger.Info("archive NODE_ID=%s open on %s (port %d) != expansion begin-port %d; allow same-host multi-node",
					nodeID, row.ListenAddress, openPort, beginPort)
				continue
			}
			if openPort <= 0 && !ClusterHasIPListenPort(csStdout, hit.target, beginPort) {
				ctx.Logger.Info("archive NODE_ID=%s open but listen port unknown; begin-port %d not yet on %s - allow expansion",
					nodeID, beginPort, hit.target)
				continue
			}
		}

// e014_check_sync_status.go - 备库同步状态检查
// 本步骤在主库连续执行 5 次归档切换（ALTER SYSTEM ARCHIVE LOG CURRENT）后，再检查备库同步状态
...
// StepE014CheckSyncStatus 备库同步状态检查步骤
func StepE014CheckSyncStatus() *runner.Step {
	return &runner.Step{
		ID:          "E-014",
		Name:        "Check Sync Status",
		Description: "On primary, run ALTER SYSTEM ARCHIVE LOG CURRENT five times, then check standby sync via yasboot cluster status",

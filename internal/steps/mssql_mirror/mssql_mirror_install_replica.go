...
// stepInstallReplica runs MS-* install steps on replica (embedded install).
func stepInstallReplica() *runner.Step {
	return &runner.Step{
		...
		Action: func(ctx *runner.StepContext) error {
			...
			mshLogPhase(ctx, "plan", fmt.Sprintf("%s install replica (%d MS steps)", ctx.CurrentStepID, len(steps)))
			if err := runner.RunEmbeddedSteps(ctx, ctx.CurrentStepID, steps); err != nil {
				return err
			}
			return nil
		},

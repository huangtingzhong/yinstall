...
func StepM008MirrorCertEndpoint() *runner.Step {
	return &runner.Step{
		ID:          "M-008",
		...
		PreCheck: func(ctx *runner.StepContext) error {
			...
			if ctx.IsForceStep() && commonmssql.ShouldBypassHACertSkip(ctx) {
				if err := commonmssql.GuardForceRecreateMirrorInfrastructure(ctx, "M-008"); err != nil {
					return err
				}
			}
			return nil
		...
		Action: func(ctx *runner.StepContext) error {
			...
			if err := dropLocalCertEndpointIfForced(ctx, commonmssql.HAEndpointMirror, "M-008", hostKey); err != nil {
				return err
			}
		...
		PostCheck: func(ctx *runner.StepContext) error {
			return commonmssql.VerifyLocalHAEndpointReady(ctx, "M-008")
		},
	}
}

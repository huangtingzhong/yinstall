...
// H-010: 解压 Oracle instantclient sqlplus + 写入 env 文件（可选）
...
// stepSetupSqlplus 解压 sqlplus 并写入环境变量
func stepSetupSqlplus() *runner.Step {
	return &runner.Step{
		...
		Action: func(ctx *runner.StepContext) error {
			...
			envFile, err := commonos.ResolveYmpOracleEnvFile(ctx, ympUser)
			if err != nil {
				return err
			}

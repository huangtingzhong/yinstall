...
// H-011: 执行 ymp.sh install --db <db_pkg> --path <instantclient_dir>
...
func ympInstallCompleted(ctx *runner.StepContext, installDir, ympUser string) (bool, error) {
	dbDir := path.Join(installDir, "yashan-migrate-platform", "db")
	dbQ := commonos.ShellSingleQuote(dbDir)
	// Package ships an empty db/ placeholder; only non-empty db counts as installed.
	r, _ := ctx.Execute(fmt.Sprintf("find %s -mindepth 1 -maxdepth 1 2>/dev/null | head -1", dbQ), false)
	...
	return r != nil && r.GetExitCode() == 0, nil
}

// stepInstallYmp 执行 YMP 安装
func stepInstallYmp() *runner.Step {
	return &runner.Step{
		...
		PreCheck: func(ctx *runner.StepContext) error {
			...
			// 判断当前安装目录是否已完成安装（db 非空或 ymp.env 存在；空 db/ 为安装包占位）
			ympUser := ctx.GetParamString("ymp_user", "ymp")
			installed, err := ympInstallCompleted(ctx, installDir, ympUser)
			if err != nil {
				return err
			}
			if installed {
				if !ctx.IsForceStep() {
					ctx.Logger.Warn("YMP already installed in %s", installDir)
					ctx.Logger.Warn("To reinstall, use: -f H-011 (in-place) or -f H-002 (wipe and reinstall)")
		...
		Action: func(ctx *runner.StepContext) error {
			...
			// Force reinstall: remove yasboot artifacts that reference this install dir.
			if isForce {
			...
			if icDir == "" {
				return fmt.Errorf("instantclient directory not found in %s, extract it first (H-008)", installDir)
			}
		...
		PostCheck: func(ctx *runner.StepContext) error {
			...
			envFile, err := commonos.ResolveYmpYasbootEnvFile(ctx, ympUser)
			if err != nil {
				return err
			}

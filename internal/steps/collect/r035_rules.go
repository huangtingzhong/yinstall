...
// StepR035CustomRules 返回 R-035 步骤：执行配置驱动的采集规则。
func StepR035CustomRules() *runner.Step {
	return &runner.Step{
		ID:       "R-035",
		...
		PreCheck: func(ctx *runner.StepContext) error {
			...
			if extra := ctx.GetParamString("collect_rules_file", ""); extra != "" {
				...
				if err != nil {
					...
					appendWarning(ctx, "R-035", fmt.Sprintf("load --rules-file %s: %v (using built-in rules only)", extra, err))
				} else {

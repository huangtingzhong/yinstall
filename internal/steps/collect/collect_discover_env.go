...
// stepDiscoverEnv 返回 R-004 步骤：自动发现 YashanDB env 文件与集群名。
// 发现结果写入 ctx.Results:
//   - env_file:     env 文件的绝对路径
//   - cluster_name: 集群名
//   - begin_port:   数据库端口（若可从 env 文件推断）
func stepDiscoverEnv() *runner.Step {
	return &runner.Step{
		...
		Action: func(ctx *runner.StepContext) error {
			...
			if err != nil {
				...
				appendWarning(ctx, fmt.Sprintf("env file not found: %v", err))
			...
			if err := writeJSON(dest, discovery); err != nil {
				appendWarning(ctx, fmt.Sprintf("write env-discovery.json: %v", err))
			}

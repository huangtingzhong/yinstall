...
// stepDbSql 返回 R-026 步骤：采集数据库 SQL 目录信息（Optional）。
func stepDbSql() *runner.Step {
	return &runner.Step{
		...
		Action: func(ctx *runner.StepContext) error {
			...
			for i, q := range queries {
				...
				output := stdout
				if err != nil {
					appendWarning(ctx, fmt.Sprintf("SQL failed: %v", err))
					if output == "" {
				...
				if werr := writeTextFile(q.dest, output); werr != nil {
					appendWarning(ctx, fmt.Sprintf("write %s: %v", q.dest, werr))
				}
			}

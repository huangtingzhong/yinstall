	...
	return &runner.Step{
		...
		Action: func(ctx *runner.StepContext) error {
			...
			if err := writeSummaryMD(rootDir, manifest, ctx); err != nil {
				appendWarning(ctx, "R-029", fmt.Sprintf("write summary.md: %v", err))
			}
			...
			if ctx.GetParamBool("archive_no_pack", false) {
				...
				if err != nil {
					appendWarning(ctx, "R-029", fmt.Sprintf("pack archive: %v", err))
					archive.NotifyPackOutcome(err.Error())
				} else if res.Skipped {
					appendWarning(ctx, "R-029", res.Message)
					...
					if err := writeJSON(filepath.Join(rootDir, "manifest.json"), manifest); err != nil {
						appendWarning(ctx, "R-029", fmt.Sprintf("update manifest with archive_path: %v", err))
					}

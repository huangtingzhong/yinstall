// g005_check_deploy_config.go - 校验 deploy 配置文件
// G-005: 检查 deploy.yml 是否存在且可读
...
// stepCheckDeployConfig 校验 deploy 配置文件
func stepCheckDeployConfig() *runner.Step {
	return &runner.Step{
		...
		Action: func(ctx *runner.StepContext) error {
			ycmLogPhase(ctx, "plan", "G-005: Check Deploy Config")
			deployFile := ctx.GetParamString("ycm_deploy_file", "/opt/ycm/etc/deploy.yml")

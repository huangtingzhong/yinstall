...
// stepConfigAutostartService 创建并启用 YCM systemd 自启动服务。
func stepConfigAutostartService() *runner.Step {
	return &runner.Step{
...
func generateServiceContent(yasadmPath string) string {
	// yasadm ycm start forks daemons and exits; without PIDFile, Type=forking leaves
	// an empty cgroup and systemd may run ExecStop immediately. oneshot+RemainAfterExit
	// matches the usual pattern for this style of start/stop script.

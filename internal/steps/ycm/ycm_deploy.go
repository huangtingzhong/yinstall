...
func lastMeaningfulDeployLine(stdout string) string {
	var last string
...
// stepDeploy 执行 YCM 初始化部署
func stepDeploy() *runner.Step {
	return &runner.Step{
		Name:        "Deploy YCM",
		...
		Action: func(ctx *runner.StepContext) error {
			...
			if result != nil {
				if result.GetExitCode() != 0 {
					stdout := strings.TrimSpace(result.GetStdout())
					stderr := strings.TrimSpace(result.GetStderr())
					if stdout != "" {
						ctx.Logger.Error("YCM deploy stdout: %s", stdout)
					}
					if stderr != "" {
						ctx.Logger.Error("YCM deploy stderr: %s", stderr)
					}
					detail := stderr
					if detail == "" {
						detail = lastMeaningfulDeployLine(stdout)
					}
					if detail != "" {
						return fmt.Errorf("YCM deploy failed with exit code %d: %s", result.GetExitCode(), detail)
					}

...
// stepSetupLocalDisk 为数据目录准备本地磁盘（LVM + 挂载）
func stepSetupLocalDisk() *runner.Step {
	return &runner.Step{
		...
		PreCheck: func(ctx *runner.StepContext) error {
			...
			if len(disks) > 0 {
				...
				// 挂载点已就绪：幂等 skip，不再因 VG 已存在失败
				if localDiskMountReady(ctx, mountPoint, user) {
					ctx.ReportPrecheckIssue(runner.PrecheckIssue{
						...
						Message:     fmt.Sprintf("mount point %s is already mounted with expected owner; apply will skip disk setup", mountPoint),
						Remediation: "If you need to rebuild, unmount it first and clean up fstab/LVM manually.",
					})
					return nil
				}

				// 检查 VG 是否尚不存在（未挂载时若 VG 已存在，Action 会尝试补挂载）
				...
				if result != nil && result.GetExitCode() == 0 {
					ctx.ReportPrecheckIssue(runner.PrecheckIssue{
						...
						Host:        ctx.Executor.Host(),
						Severity:    runner.PrecheckSeverityInfo,
						Code:        "PC.OS.DISK.VG_EXISTS",
						Message:     fmt.Sprintf("VG '%s' already exists; apply will mount existing LV if present", vgName),
						Remediation: "Ensure LV and filesystem match expected layout, or remove VG to recreate.",
					})
					return nil
				}
		...
		Action: func(ctx *runner.StepContext) error {
			...
			if len(disks) == 0 {
				...
				if localDiskDirReady(ctx, mountPoint, user) && !ctx.IsForceStep() {
					ctx.Logger.Info("Directory %s already ready (owner %s), skipping", mountPoint, user)
					return nil
				}
			...
			// 若挂载点已挂载且属主正确则跳过创建流程
			if !ctx.IsForceStep() && localDiskMountReady(ctx, mountPoint, user) {
				ctx.Logger.Info("Mount point %s already mounted with owner %s, skipping", mountPoint, user)
				return nil
			}
			...
			// VG 已存在：尝试补挂载现有 LV，不再报错失败
			result, _ := ctx.Execute(fmt.Sprintf("vgs %s 2>/dev/null", vgName), false)
			if result != nil && result.GetExitCode() == 0 {
				lvCheck, _ := ctx.Execute(fmt.Sprintf("lvs %s 2>/dev/null", lvPath), false)
				if lvCheck == nil || lvCheck.GetExitCode() != 0 {
					return fmt.Errorf("VG '%s' already exists but LV '%s' not found; remove VG or create LV manually", vgName, lvPath)
				}
				ctx.Logger.Info("VG/LV already exist, ensuring mount for %s", lvPath)
				return ensureLocalDiskMounted(ctx, lvPath, mountPoint, mountOpts, user, group)
			}
...
func localDiskDirReady(ctx *runner.StepContext, mountPoint, user string) bool {
	r, _ := ctx.Execute(fmt.Sprintf("test -d %s && stat -c '%%U' %s", mountPoint, mountPoint), false)
	if r == nil || r.GetExitCode() != 0 {
		return false
	}
	return strings.TrimSpace(r.GetStdout()) == user
}

func localDiskMountReady(ctx *runner.StepContext, mountPoint, user string) bool {
	r, _ := ctx.Execute(fmt.Sprintf("mountpoint -q %s 2>/dev/null", mountPoint), false)
	if r == nil || r.GetExitCode() != 0 {
		return false
	}
	return localDiskDirReady(ctx, mountPoint, user)
}

func ensureLocalDiskMounted(ctx *runner.StepContext, lvPath, mountPoint, mountOpts, user, group string) error {
	ctx.Execute(fmt.Sprintf("mkdir -p %s", mountPoint), true)
	mp, _ := ctx.Execute(fmt.Sprintf("mountpoint -q %s 2>/dev/null", mountPoint), false)
	if mp == nil || mp.GetExitCode() != 0 {

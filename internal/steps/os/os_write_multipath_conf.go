...
// stepWriteMultipathConf 写入 multipath.conf（YAC）
// 使用 B-025 收集的 WWID 信息生成 multipath.conf
func stepWriteMultipathConf() *runner.Step {
	return &runner.Step{
        ...
        alias    %s
			...
			// 收集 systemdg 磁盘
			if systemdg != nil {
				ctx.Logger.Info("Processing systemdg '%s':", DiskGroupConfigLabel(systemdg, "systemdg"))
			...
			// 收集 datadg 磁盘
			if datadg != nil {
				ctx.Logger.Info("Processing datadg '%s':", DiskGroupConfigLabel(datadg, "datadg"))
			...
			if archdg != nil && (datadg == nil || DiskGroupConfigsDistinct(archdg, datadg)) {
				ctx.Logger.Info("Processing archdg '%s':", DiskGroupConfigLabel(archdg, "archdg"))
...
`, strings.Join(multipathEntries, "\n"))
			...
			if !ctx.IsForceStep() {
				existing, _ := ctx.Execute(fmt.Sprintf("cat %s 2>/dev/null || true", confPath), false)
				cur := ""
				if existing != nil {
					cur = existing.GetStdout()
				}
				if commonos.TextContentEqual(cur, config) {
					ctx.Logger.Info("%s already configured, skipping write (use -f %s to force)", confPath, ctx.CurrentStepID)
					osLogPhase(ctx, "skip", "already_configured=multipath_conf")
					return nil
				}
			}

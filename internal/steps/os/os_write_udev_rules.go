...
// FormatYfsIDWWNUdevRule 生成基于 ID_WWN 的 yfs udev 规则行（假 mapper→nvme / 裸 nvme 原生多路径）。
		...
		Action: func(ctx *runner.StepContext) error {
			...
			processDisks := func(dg *DiskGroupConfig, prefix string) error {
				...
				for i, disk := range dg.Disks {
					...
					} else if ctx.GetParamBool("yac_udev_use_id_wwn", false) {
						idWWN, err := commonos.GetDiskIDWWN(ctx, disk)
						if err != nil {
							return fmt.Errorf("ID_WWN udev: %w", err)
						}
						rule := FormatYfsIDWWNUdevRule(idWWN, alias, owner, group, mode)
						rules = append(rules, rule)
						ctx.Logger.Info("  Generated ID_WWN-based rule for disk %s -> /dev/yfs/%s (ID_WWN: %s)", disk, alias, idWWN)

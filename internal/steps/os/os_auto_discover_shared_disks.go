...
// stepAutoDiscoverSharedDisks YAC 模式下自动发现各节点共享磁盘
// 当 yac_systemdg、yac_datadg、yac_archdg 都未配置时，自动发现共享磁盘
func stepAutoDiscoverSharedDisks() *runner.Step {
	return &runner.Step{
...
func scanDisksOnNode(ctx *runner.StepContext, diskPattern string, excludeDisks []string) ([]*DiskInfo, error) {
	...
	for _, line := range lines {
		...
		// 获取 WWID（复用 commonos 的 NVMe/SCSI 统一实现）
		wwid, err := commonos.GetDiskWWID(ctx, diskPath)
		if err != nil || wwid == "" {
...
// generateDiskGroupParam 生成纯路径列表参数（dgName 保留兼容调用方，已忽略）。

// b006_configure_umask.go - 配置用户 umask
// B-006: 在用户的 .bashrc 中设置 umask 022
...
// stepConfigureUmask 为产品用户配置 umask
func stepConfigureUmask() *runner.Step {
	return &runner.Step{
		...
		PreCheck: func(ctx *runner.StepContext) error {
			...
			homeDir, err := commonos.GetUserHomeDir(ctx, osUser)
			if err != nil {
				return err
			}

...
// stepSetHostname 配置各节点主机名（及 /etc/hosts 托管块）
		...
		Action: func(ctx *runner.StepContext) error {
			...
			if len(nodes) > 0 {
				...
				for _, th := range hosts {
					...
					hctx := ctx.ForHost(th)
					changed, err := commonos.EnsureManagedHostsBlock(hctx, entries)
					if err != nil {
						return fmt.Errorf("[%s] failed to update /etc/hosts: %w", th.Host, err)
					}
					if !changed {
						ctx.Logger.Info("[%s] /etc/hosts managed block already configured, skipping", th.Host)
					} else {
						ctx.Logger.Info("[%s] /etc/hosts managed block updated", th.Host)
					}
					osLogPhase(hctx, "host-done", fmt.Sprintf("host=%s op=etc-hosts-block changed=%v", th.Host, changed))
				}
			}

...
// StepS08NETBench 返回 S-08 步骤：per-host ping；YAC 为节点间互 ping。
func StepS08NETBench() *runner.Step {
	return &runner.Step{
		ID:       "S-08",
...
func s08RunPingBench(ctx *runner.StepContext, pingTarget string) error {
	...
	if err != nil {
		appendWarning(ctx, "S-08", fmt.Sprintf("ping %s failed: %v", pingTarget, err))
		pingTxt += "\nERROR: " + err.Error()
	...
	if err2 := writeTextFile(destPath, pingTxt+"\n"); err2 != nil {
		appendWarning(ctx, "S-08", "write "+destName+": "+err2.Error())
	}
...
func RunIperf3YAC(serverCtx *runner.StepContext, clientCtxs []*runner.StepContext) error {
	...
	if _, err := stressExecute(serverCtx, startCmd, false, 30*time.Second); err != nil {
		appendWarning(serverCtx, "S-08", fmt.Sprintf("iperf3 server start: %v", err))
		return err
	}
	...
	for i, clientCtx := range clientCtxs {
		...
		if clientErr != nil {
			...
			appendWarning(clientCtx, "S-08", fmt.Sprintf("iperf3 client %s: %v", clientHost, clientErr))
		...
		if err := writeTextFile(perClientFile, report); err != nil {
			appendWarning(clientCtx, "S-08", "write "+filepath.Base(perClientFile)+": "+err.Error())
		}
	...
	if err := writeTextFile(filepath.Join(yacDir, "iperf3_report.txt"), summary.String()); err != nil {
		appendWarning(serverCtx, "S-08", "write iperf3_report.txt: "+err.Error())
	}
	if err := writeJSON(filepath.Join(yacDir, "iperf3_meta.json"), map[string]interface{}{
		...
		appendWarning(serverCtx, "S-08", "write iperf3_meta.json: "+err.Error())
	}

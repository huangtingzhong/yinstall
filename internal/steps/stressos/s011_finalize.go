...
// StepS11Finalize 返回 S-11 步骤：生成最终摘要文件。
func StepS11Finalize() *runner.Step {
	return &runner.Step{
		ID:   "S-11",
		...
		Action: func(ctx *runner.StepContext) error {
			...
			if err := s11WriteSummaryMD(rootDir, manifest, ctx); err != nil {
				appendWarning(ctx, "S-11", fmt.Sprintf("write summary.md: %v", err))
			}
			...
			if ctx.GetParamBool("archive_no_pack", false) {
				...
				if err != nil {
					appendWarning(ctx, "S-11", fmt.Sprintf("pack archive: %v", err))
					archive.NotifyPackOutcome(err.Error())
				} else if res.Skipped {
					appendWarning(ctx, "S-11", res.Message)
					...
					if err := writeJSON(filepath.Join(rootDir, "manifest.json"), manifest); err != nil {
						appendWarning(ctx, "S-11", fmt.Sprintf("update manifest with archive_path: %v", err))
					}

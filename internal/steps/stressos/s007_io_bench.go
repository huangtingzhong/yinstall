...
// StepS07IOBench 返回 S-07 步骤：IO 压测。
func StepS07IOBench() *runner.Step {
	return &runner.Step{
		ID:       "S-07",
		...
		Action: func(ctx *runner.StepContext) error {
			...
			for _, job := range jobs {
				...
				if err != nil {
					appendWarning(ctx, "S-07", fmt.Sprintf("read fio template %s: %v", job.jobFile, err))
					continue
				}
				...
				if fioErr != nil {
					appendWarning(ctx, "S-07", fmt.Sprintf("fio %s failed: %v", job.jobFile, fioErr))
					txtContent += "\nERROR: " + fioErr.Error()
				}
				if err2 := writeTextFile(txtDest, txtContent+"\n"); err2 != nil {
					appendWarning(ctx, "S-07", "write "+txtDest+": "+err2.Error())
				}
...
func s07DownloadFIOJSON(ctx *runner.StepContext, remotePath, localDest string) {
	...
	if err2 := writeTextFile(localDest, content); err2 != nil {
		appendWarning(ctx, "S-07", "write fio json "+localDest+": "+err2.Error())
	}
}

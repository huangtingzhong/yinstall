...
//	--os-yum-mode=local  先 EnsureLocalISORepo（挂载 ISO）再安装；
...
// stepInstallDeps 返回 S-03 步骤：安装压测依赖工具。
func stepInstallDeps() *runner.Step {
	return &runner.Step{
		...
		PreCheck: func(ctx *runner.StepContext) error {
			...
			yumMode := getStr(ctx, "os_yum_mode", "")
		...
		Action: func(ctx *runner.StepContext) error {
			...
			// ── 1. 挂载本地 ISO（local 模式）──────────────────────────────
			if commonos.IsLocalYumMode(yumMode) {
				add("--- ISO repo setup ---")
				if err := commonos.EnsureLocalISORepo(ctx); err != nil {
					...
					appendWarning(ctx, fmt.Sprintf("EnsureLocalISORepo: %v", err))
				} else {
			...
			if err := s03InstallFIOBuildDeps(ctx, pkgManager, yumMode, isRHEL8); err != nil {
				...
				appendWarning(ctx, fmt.Sprintf("fio build deps: %v", err))
			} else {
			...
			for _, tool := range s03Tools {
				...
				if tool.dedicatedPkg && tool.name == "numactl" {
					...
					if ok {
						...
						appendWarning(ctx, "failed to install numactl packages")
					}
					add("")
					continue
				}
				...
				if tool.hasSourceBuild {
					...
					if srcErr != nil {
						...
						if tool.optional {
							...
							appendWarning(ctx, fmt.Sprintf("%s source build failed: %v", tool.name, srcErr))
						}
					} else {
				...
				if tool.optional {
					add(fmt.Sprintf("  %-12s SKIP (optional, not available)", tool.name))
					if tool.epolOnly && commonos.IsLocalYumMode(yumMode) {
						add(fmt.Sprintf("  NOTE: %s is typically in EPEL and may not be in the base ISO; "+
							"install manually or use default system repos (--os-yum-mode empty)", tool.name))
					}
				} else {
					...
					appendWarning(ctx, fmt.Sprintf("failed to install %s", tool.name))
				}
			}
			add("")
			...
			if ok, method := s03InstallTool(ctx, "sysbench", pkgManager, yumMode, isRHEL8); ok {
				...
				if err := s03BuildSysbenchFromSource(ctx, pkgManager, yumMode, isRHEL8); err != nil {
					...
					appendWarning(ctx, fmt.Sprintf("sysbench source build failed: %v", err))
				} else {

...
// stepPostSnapshot 返回 S-10 步骤：终止后台 OS 性能采集并下载结果。
func stepPostSnapshot() *runner.Step {
	return &runner.Step{
		Name: "Stop background OS performance collection",
		Action: func(ctx *runner.StepContext) error {
			remotePerfDir := s10ResolvePerfDir(ctx)
			if remotePerfDir == "" {
				appendWarning(ctx, "perf_remote_dir not found; S-04 may not have run or breadcrumb file missing")
				return nil
			}
...
`, remotePerfDir)
			...
			for _, name := range logFiles {
				...
				if execErr != nil {
					appendWarning(ctx, fmt.Sprintf("read remote %s: %v", remotePath, execErr))
					continue
				}
				...
				if err := writeTextFile(localPath, content); err != nil {
					appendWarning(ctx, fmt.Sprintf("write %s: %v", localPath, err))
					continue
				}
				downloaded++
			...
			if _, cleanErr := stressExecute(ctx,
				...
				appendWarning(ctx, fmt.Sprintf("cleanup %s: %v", remotePerfDir, cleanErr))
			}

...
// StepS09RuntimeMetrics 返回 S-09 步骤：运行时指标采集。
func StepS09RuntimeMetrics() *runner.Step {
	return &runner.Step{
		ID:   "S-09",
		...
		Action: func(ctx *runner.StepContext) error {
			...
			for _, item := range scripts {
				...
				if err != nil {
					appendWarning(ctx, "S-09", fmt.Sprintf("read %s: %v", item.scriptFile, err))
					continue
				}

				out, runErr := stressRunShell(ctx, content, false, timeout)
				if runErr != nil {
					appendWarning(ctx, "S-09", fmt.Sprintf("%s failed: %v", item.scriptFile, runErr))
					out += "\nERROR: " + runErr.Error()
				}

				destPath := filepath.Join(runtimeDir, item.destFile)
				if err2 := writeTextFile(destPath, out+"\n"); err2 != nil {
					appendWarning(ctx, "S-09", "write "+destPath+": "+err2.Error())
				}

...
// StepS05CPUBench 返回 S-05 步骤：CPU 压测。
func StepS05CPUBench() *runner.Step {
	return &runner.Step{
		ID:       "S-05",
...
func s05RunNUMACPUBench(ctx *runner.StepContext, cpuDir string, timeout time.Duration,
	...
	if !s03ToolAvailable(ctx, "numactl") {
		appendWarning(ctx, "S-05", "numactl not available; skip NUMA node CPU benchmark")
		return
	}

	hw, ok := s05FetchNUMATopology(ctx)
	if !ok || strings.TrimSpace(hw) == "" {
		appendWarning(ctx, "S-05", "cannot read numactl --hardware; skip NUMA node CPU benchmark")
		return
	}
	if err := writeTextFile(filepath.Join(cpuDir, "numa_hardware.txt"),
		"=== numactl --hardware ===\n"+hw+"\n"); err != nil {
		appendWarning(ctx, "S-05", "write numa_hardware.txt: "+err.Error())
	}

	nodeCPUs := s05ParseNUMANodeCPUCount(hw, numaNode)
	if nodeCPUs <= 0 {
		appendWarning(ctx, "S-05",
			fmt.Sprintf("NUMA node %d has no CPUs in topology; skip NUMA CPU benchmark", numaNode))
		return
	}
	node2x := nodeCPUs * 2
...
func s05RunSysbenchCPU(ctx *runner.StepContext, cpuDir string, timeout time.Duration,
	...
	if err != nil {
		appendWarning(ctx, "S-05", fmt.Sprintf("%s failed: %v", header, err))
		body += "\nERROR: " + err.Error()
	...
	if err2 := writeTextFile(dest, "=== "+header+" ===\n"+body+"\n"); err2 != nil {
		appendWarning(ctx, "S-05", "write "+outFile+": "+err2.Error())
	}
}

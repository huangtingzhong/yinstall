...
// StepS04StartPerfCollect 返回 S-04 步骤：后台启动 OS 性能并行采集。
func StepS04StartPerfCollect() *runner.Step {
	return &runner.Step{
		ID:   "S-04",
		...
		Action: func(ctx *runner.StepContext) error {
			...
			if err != nil {
				appendWarning(ctx, "S-04",
					fmt.Sprintf("start perf collection may have partial failures: %v", err))
			}

...
// StepA003PlanReplicaInstall resolves replica setup media matching primary version.
func StepA003PlanReplicaInstall() *runner.Step {
	return &runner.Step{
		ID:          "A-003",
		...
		PreCheck: func(ctx *runner.StepContext) error {
			if commonmssql.IsPrimaryHost(ctx) {
				return runner.NewStepSkippedError("A-003 runs on replica only")
			}
			stage := haStage(ctx)
		...
		Action: func(ctx *runner.StepContext) error {
			...
			if !ok {
				return fmt.Errorf("primary instance info missing; run A-002 first")
			}
			mshLogPhase(ctx, "plan", "A-003 plan replica install")

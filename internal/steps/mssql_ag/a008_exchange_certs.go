...
// StepA008ExchangeHADRCerts exchanges HADR certificates between the current
...
func StepA008ExchangeHADRCerts() *runner.Step {
	return &runner.Step{
		ID:          "A-008",
		...
		Action: func(ctx *runner.StepContext) error {
			...
			for _, partnerKey := range haPeerHosts(ctx) {
				...
				if err := ensurePartnerCertImported(ctx, kind, "A-008", partnerKey, partnerCertLocal); err != nil {
					return err
				}
			}
			return nil

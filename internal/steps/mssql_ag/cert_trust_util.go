...
func ensurePartnerCertImported(ctx *runner.StepContext, kind commonmssql.HAEndpointKind, stepID, partnerKey, cerLocal string) error {
	...
	if dbThumb != "" && dbThumb == fileThumb && hasLogin {
		...
		// Only guard when the cert actually exists but thumbprint mismatches
		// (an existing partner's cert changed). When dbThumb is empty (new
		// partner, cert never imported), allow creation without force.
		mismatch := dbThumb != "" && fileThumb != "" && dbThumb != fileThumb
		ctx.Logger.Info("cert_trust: partner=%s cert=%s dbThumb=%q fileThumb=%q hasLogin=%v mismatch=%v protected=%v",
			partnerKey, certName, dbThumb, fileThumb, hasLogin, mismatch, hasLogin && dbThumb != "")
		if mismatch {
...
func dropLocalCertEndpointIfForced(ctx *runner.StepContext, kind commonmssql.HAEndpointKind, stepID, hostKey string) error {
	...
	if kind == commonmssql.HAEndpointHADR {
		...
		if any {
			return fmt.Errorf("%s: cannot force-recreate HADR cert/endpoint while AG databases are active on this instance; remove AG replicas from this node first", stepID)
		}
	}

...
// StepC012BDisableArchivelog 按需将 yashandb.toml 中 ISARCHIVELOG 改为 false（关闭归档）
func StepC012BDisableArchivelog() *runner.Step {
	return &runner.Step{
		ID:          "C-012B",
		...
		PreCheck: func(ctx *runner.StepContext) error {
			...
			if result == nil || result.GetExitCode() != 0 {
				return fmt.Errorf("cluster config not found at %s (run C-011 first)", configPath)
			}
			return nil
		},

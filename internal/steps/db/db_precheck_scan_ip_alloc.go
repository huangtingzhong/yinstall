...
func RunScanIPAllocation(hosts []HostExec, params map[string]interface{}, logger *logging.Logger) error {
	...
	logger.ConsoleWithType(StepIDByName("Write Cluster Hosts"), "Local SCAN IP Allocation", firstHost, "start", "", "", 0)
	...
	if scanIPsRaw != "" {
		...
		logger.ConsoleWithType(StepIDByName("Write Cluster Hosts"), "Local SCAN IP Allocation", firstHost, "success", "",

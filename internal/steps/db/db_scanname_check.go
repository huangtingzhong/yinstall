...
// stepScannameCheck SCAN 名解析与网段校验步骤；实际逻辑由 RunScanNameResolveAndSubnetCheck 在 db 命令中调用
func stepScannameCheck() *runner.Step {
	return &runner.Step{
...
func RunScanNameResolveAndSubnetCheck(hosts []HostExec, params map[string]interface{}, logger *logging.Logger) error {
	...
	logger.ConsoleWithType(StepIDByName("Resolve SCAN Name and Check Subnet"), "Resolve SCAN Name and Check Subnet", firstHost, "start", "", "", 0)
	...
	logger.ConsoleWithType(StepIDByName("Resolve SCAN Name and Check Subnet"), "Resolve SCAN Name and Check Subnet", firstHost, "success", "", fmt.Sprintf("IPs: %v", resolvedIPs), time.Duration(0))
	return nil
}

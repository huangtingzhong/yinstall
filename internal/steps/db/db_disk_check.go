...
// stepDiskCheck 校验共享盘对各节点可见性及权限
func stepDiskCheck() *runner.Step {
	return &runner.Step{
		...
		Action: func(ctx *runner.StepContext) error {
			...
			if needMultipath && !skipPathRewrite {
				...
				updateDiskGroupParam := func(dgStr string) string {
					return MapYACDiskGroupParam(dgStr, func(disk string, _ int) string {
						if mapping, exists := referenceMappings[disk]; exists {
							return mapping.MultipathDisk
						}
						return disk
					})
				}

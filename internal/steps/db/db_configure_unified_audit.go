...
// stepConfigureUnifiedAudit 开启统一审计、创建策略与清理作业（--db-unified-audit，默认关闭）。
// CDB 模式：UNIFIED_AUDITING 参数与调度作业在 CDB$ROOT；审计策略在每个 --db-pdb 内创建。
// 非 CDB：参数、策略与作业均在实例内执行。
func stepConfigureUnifiedAudit() *runner.Step {
	return &runner.Step{
...
// resolveDBEnvFile 优先使用 C-023 写入的 env_file，否则按 getent home 与端口推导。
func resolveDBEnvFile(ctx *runner.StepContext, hctx *runner.StepContext) (string, error) {
	user := hctx.GetParamString("os_user", "yashan")
	beginPort := hctx.GetParamInt("db_begin_port", 1688)
	return commonos.ResolveEnvFileForUser(ctx, hctx, user, beginPort)
}

...
func RunNetworkValidation(hosts []HostExec, params map[string]interface{}, logger *logging.Logger) error {
	...
	logger.ConsoleWithType(FirstStepID(), "Network CIDR Validation", firstHost, "start", "", "", 0)
	...
	logger.ConsoleWithType(FirstStepID(), "Network CIDR Validation", firstHost, "success", "",
		fmt.Sprintf("public=%s inter=%s", detectedPublic, detectedInter), time.Duration(0))
	return nil
}

// ValidateDBReplicaCIDR 校验 --db-replica-cidr：空则跳过；非空则各节点须存在该网段网卡。
func ValidateDBReplicaCIDR(hosts []HostExec, params map[string]interface{}, logger *logging.Logger) error {
	replicaCIDR := strings.TrimSpace(getParamString(params, "db_replica_cidr", ""))
	if replicaCIDR == "" {
		return nil
	}
	if logger != nil {
		logger.Info("Validating db_replica_cidr=%s on %d host(s)...", replicaCIDR, len(hosts))
	}
	if _, err := validateHostCIDR(hosts, replicaCIDR, "db-replica-cidr", logger); err != nil {
		return fmt.Errorf("db-replica-cidr validation failed: %w", err)
	}
	if logger != nil {
		logger.Info("db_replica_cidr validated: %s", replicaCIDR)
	}
	return nil
}
...
func validateInterCIDR(hosts []HostExec, interCIDR string, logger *logging.Logger) (string, error) {
	return validateHostCIDR(hosts, interCIDR, "inter-cidr", logger)
}

// ValidateHostCIDR 校验 CIDR 在各节点均存在且网卡名一致（导出供 standby CE 等复用）。
func ValidateHostCIDR(hosts []HostExec, cidr, label string, logger *logging.Logger) (string, error) {
	return validateHostCIDR(hosts, cidr, label, logger)
}

// ValidateYACNetworksOnHosts 在目标节点上校验 inter-cidr（必填）与可选 public-network 网卡存在且网卡名一致。
// 不做空 CIDR 自动探测（standby CE 要求显式 --yac-inter-cidr）。
func ValidateYACNetworksOnHosts(hosts []HostExec, interCIDR, publicNetwork string, logger *logging.Logger) error {
	if len(hosts) == 0 {
		return fmt.Errorf("no hosts for YAC network validation")
	}
	interCIDR = strings.TrimSpace(interCIDR)
	if interCIDR == "" {
		return fmt.Errorf("--yac-inter-cidr is required")
	}
	if _, err := validateHostCIDR(hosts, interCIDR, "inter-cidr", logger); err != nil {
		return err
	}
	publicNetwork = strings.TrimSpace(publicNetwork)
	if publicNetwork == "" {
		if logger != nil {
			logger.Info("yac_public_network empty: skip public-network interface check on targets")
		}
		return nil
	}
	if _, err := validateHostCIDR(hosts, publicNetwork, "public-network", logger); err != nil {
		return err
	}
	return nil
}

// validateHostCIDR 校验 CIDR 在各节点均存在且网卡名一致；label 用于日志/错误文案（如 inter-cidr、db-replica-cidr）。
func validateHostCIDR(hosts []HostExec, cidr, label string, logger *logging.Logger) (string, error) {
	_, _, err := net.ParseCIDR(cidr)
	if err != nil {
		return "", fmt.Errorf("invalid %s CIDR format %q: %w", label, cidr, err)
	}
	...
	for _, h := range hosts {
		...
		for _, info := range ifaces {
			if sameCIDR(info.CIDR, cidr) {
				matchedIface = info.Name
				break
			}
		}
		if matchedIface == "" {
			return "", fmt.Errorf("%s %s not found on node %s (no interface matches this subnet)", label, cidr, h.Host)
		}
		ifaceNames = append(ifaceNames, matchedIface)
		if logger != nil {
			logger.Info("Node %s: %s %s matched interface %s", h.Host, label, cidr, matchedIface)
		}
	}
	...
	for i := 1; i < len(ifaceNames); i++ {
		if ifaceNames[i] != refIface {
			...
			return "", fmt.Errorf("%s %s interface mismatch: %s", label, cidr, strings.Join(details, ", "))
		}
	}

	if logger != nil {
		logger.Info("%s %s validated: interface %s on all %d nodes", label, cidr, refIface, len(hosts))
	}
	return cidr, nil
}

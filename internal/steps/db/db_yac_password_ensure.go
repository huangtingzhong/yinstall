...
func RunYACProductUserPasswordEnsure(hosts []HostExec, params map[string]interface{}, logger *logging.Logger, precheckMode bool) error {
	...
	if !getParamBool(params, "yac_ensure_os_password", true) {
		logger.Info("%s: yac_ensure_os_password=false, skipping product user password check", FirstStepID())
		return nil
	}
	...
	logger.ConsoleWithType(FirstStepID(), "Ensure product user password (YAC)", firstHost, "start", "", "", 0)
	dbLogPhase(&runner.StepContext{Logger: logger, CurrentStepID: FirstStepID()}, "plan",
	...
	for _, h := range hosts {
		...
		if ok {
			logger.Info("%s: SSH password OK for %s@%s", FirstStepID(), user, h.Host)
			dbLogPhase(hctx, "op-done", "probe=ok")
			continue
		}
		dbLogPhase(hctx, "op-fail", fmt.Sprintf("probe=%s", probeNote))
		logger.Warn("%s: product user %q cannot SSH to %s with --os-user-password (will reset via root/sudo if allowed)", FirstStepID(), user, h.Host)
		if !autoFix {
	...
	for i, host := range fixHosts {
		...
		logger.Info("%s: resetting password for %s@%s (via %s)", FirstStepID(), user, host, via)
		...
		logger.Info("%s: password reset and verified for %s@%s", FirstStepID(), user, host)
	}
	...
	logger.ConsoleWithType(FirstStepID(), "Ensure product user password (YAC)", firstHost, "success", "", "", 0)
	logger.Info("%s: product user password ready on all YAC nodes", FirstStepID())
	return nil
}
...
func probeProductUserPasswordSSH(host string, port int, user, password string, logger *logging.Logger) (bool, string, error) {
	return runner.ProbePasswordSSH(logger, FirstStepID(), ssh.Config{
		Host:     host,
		Port:     port,
		User:     user,
		Password: password,
...
func probeYACPasswordMeshFromFirstNode(first HostExec, params map[string]interface{}, user, password string, targetIPs []string, logger *logging.Logger) error {
	...
	if result != nil && strings.Contains(result.GetStdout(), "NOT_FOUND") {
		rootCtx.Logger.Warn(fmt.Sprintf("%s:", FirstStepID())+" sshpass not found on %s; skip mesh SSH probe (yasboot ce gen may still require sshpass on first node)", first.Host)
	...
	logger.Info(fmt.Sprintf("%s:", FirstStepID())+" mesh SSH probe OK from %s to %d target(s)", first.Host, len(targetIPs))
	return nil
}

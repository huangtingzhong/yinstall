...
// yashanCharacterSetCanonical：规范化键（大写、无连字符）→ 写入 yashandb.toml 的 CHARACTER_SET。
// 已按 23.5.2 真机验证：UTF8/GBK/GB18030 可用；LATIN1/UTF8MB4 触发 YAS-00216，已移除。
...
const yashanCharacterSetList = "UTF8, GBK, ASCII, GB18030, BINARY, UTF8MB3"
...
// CanonicalYashanCharacterSet 导出供 CLI/单测校验字符集名。
func CanonicalYashanCharacterSet(raw string) (string, error) {
	return canonicalYashanCharacterSet(raw)
}

// stepSetCharacterSet 在集群 TOML 中设置字符集
func stepSetCharacterSet() *runner.Step {
	return &runner.Step{

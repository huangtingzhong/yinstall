...
func RunYACUdevDiskDiscovery(hosts []HostExec, params map[string]interface{}, logger *logging.Logger) error {
	...
	if !ShouldRunYACUdevDiskDiscovery(params) {
		logger.Info("%s: skipped (disk groups configured or auto-discover disabled)", stepID)
		return nil
	}

	firstHost := hosts[0].Host
	logger.ConsoleWithType(stepID, "YAC Udev Disk Discovery", firstHost, "start", "", "", 0)
	...
	logger.Info("%s: discovering YAC disk groups (root=%s fallback_mapper=%v)", stepID, root, fallbackMapper)
	if len(excludes) > 0 {
		logger.Info("%s: exclude disks: %v", stepID, excludes)
	}
	logger.Debug(logging.LogEntry{
		Host: firstHost, StepID: stepID, Level: "debug",
		Phase:   "discover-plan",
	...
	logger.Info("%s discovery results:", stepID)
	...
	logger.ConsoleWithType(stepID, "YAC Udev Disk Discovery", firstHost, "success", "",

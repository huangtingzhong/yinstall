...
type PDBSpec struct {
	...
	UsersSize           string
	TablespaceSpecified bool // true when datafile/size (or official aliases) set on --db-pdb
	Archivelog          *bool
...
func parseOnePDBSpec(specStr string, index int) (PDBSpec, error) {
	...
	if v := strings.TrimSpace(kv[pdbParamDatafile]); v != "" {
		...
		spec.TablespaceSpecified = true
	}
	if v := strings.TrimSpace(kv[pdbParamSize]); v != "" {
		spec.UsersSize = v
		spec.TablespaceSpecified = true
	}
	if spec.TablespaceSpecified {
		if spec.UsersDatafile == "" {
			spec.UsersDatafile = "users_" + strings.ToLower(name)
		}
		if spec.UsersSize == "" {
			spec.UsersSize = defaultPDBUsersSize
		}
	}
